...
`/workspace/media-accounts` 已作为“账号报白”页面接入用户媒体账号接口，并迁移 TDesign Starter 筛选列表结构：账号报白 Dialog、新增账号、平台报白方式与状态表格、客户端筛选和分页、加载/空数据/错误状态。媒体平台、账号 ID、账号名称和账号主页链接均为必填，主页链接必须使用 HTTPS。用户端只展示未提交、审核中、已加白、未通过四种业务状态；技术提交失败统一显示为审核中，不展示错误码、错误原因或管理员操作，也不提供删除入口。

`/workspace/drama` 已接入 `/api/user/promotion/dramas`，沿用账号报白页面的 Starter 筛选列表结构：短剧标题和语言筛选、合并展示海报/中英文名称/标签的短剧信息列、平台/分类/语言/推广说明/发布时间表格以及服务端分页。接口只返回已上架且甲方在线的短剧，并按甲方 `remoteCreatedAt` 发布时间倒序排列。“创建推广任务”操作在当前页面打开所选短剧详情抽屉，抽屉展示封面、标题和简介，不展示推广说明；创建弹窗只选择媒体平台和可选推广名称，一次请求为每个媒体平台固定生成落地页和 OneLink。所有变体生成成功后才进入 `/workspace/promotion-links`，请求失败或批次不完整时保留当前创建弹窗。

短剧详情页使用浏览器直接下载甲方提供的原始素材资源。单集下载直接保存当前免费剧集；“下载全部”按集数依次触发当前短剧全部可用免费剧集，文件名按资源 URL 保留 `.mp4` 或 `.m3u8` 类型，不创建后端下载任务、不使用 FFmpeg，也不生成 ZIP。浏览器下载进度由浏览器自身管理。播放失败直接提示，不自动请求后端刷新资源。

`/workspace/promotion-links` 菜单显示为“推广任务”，按 GoodShort 口令一行只读展示创建时间、推广名称、短剧、媒体平台、口令、点击数、归因用户数、新注册人数、新充值人数、新会员人数、充值用户数和订单数；推广 URL 使用独立的“OneLink”和“落地页”两列，两列宽度均为 320px，完整 URL 在列内自然换行且不省略，同时提供打开、复制操作，悬停可查看完整地址，历史单变体在对应列显示“暂无”。转化指标由后端关联 GoodShort 日报后累计返回，同一口令只展示一份；页面不展示链接类型、批次、状态、失败原因、操作列或充值金额，也不提供重试、修改或重新提交。

`/workspace/orders` 已接入 `/api/user/promotion/orders`，按月份分页展示当前登录用户已归因订单的甲方订单号、未支付/已支付/已退款状态、支付时间、甲方推广口令 `searchCode` 和“我的收益”。页面不提供订单导出，不展示完整订单金额或内部佣金状态，只展示后端收益结果，不在前端重算佣金；当前不提供独立佣金页面。

`/workspace/profile` 通过工作区 Header 的头像菜单进入，不显示在侧边栏。页面使用 `/api/user/auth/me` 展示本人资料；用户名下方只读显示学员类型（`0=基础用户`、`1=基础学员`），不会出现在编辑资料表单。进入编辑态后，昵称、真实姓名、微信号、手机号码和电子邮箱直接在基本信息区域切换为输入框，并通过 `/api/user/auth/profile` 保存。手机号或电子邮箱变更后旧会话失效，页面清除本地会话并返回登录页；仅修改其他资料时当前会话保持有效。头像通过 `/api/user/auth/avatar` 上传 JPG/PNG/WebP 文件，头像不显示蓝色外圈。页面不显示实名认证；“基本信息”和“安全设置”是椭圆形白色切换按钮，安全设置内容不嵌套卡片。资料或头像更新成功后同步刷新 Header 共用的用户缓存。用户编号、注册时间和最近登录信息保持只读。密码修改在安全设置中先展示当前绑定手机号的脱敏值，调用 `/api/user/auth/password/change/code` 发码并调用 `/api/user/auth/password/change/verify` 校验；验证通过后才显示新密码表单，10 分钟一次性凭证只保存在组件内存。改密成功后清除本地会话、查询缓存并返回登录页重新登录；未绑定手机号时提示先绑定。
