...
The login route is rendered as a full-page Starter-style surface at `/login`. The login page, browser title, and workspace sidebar use the `卡司短剧推广平台` brand; on screens narrower than 480px, the login header keeps the brand text and hides the full logo image to avoid horizontal crowding. The authenticated workspace shell is available under `/workspace` and uses the migrated TDesign Starter SideLayout structure: a 232px sidebar, sticky header, breadcrumb-aware page container, footer, and responsive menu collapse. The footer stays at the bottom of the workspace viewport when page content is short. The workspace home page displays a centered welcome message using the signed-in user's nickname, with `用户` as the fallback. The workspace header keeps the message and user menus; help documentation, page settings, and download center entries are not shown.
...
认证 API 已提供手机号注册验证码、验证码登录和找回密码验证码调用；邮箱密码登录保持可用，邮箱验证码暂缓。
