...
`/workspace/promotion-links` 菜单显示为“推广任务”，按 GoodShort 口令一行只读展示创建时间、推广名称、短剧、媒体平台、口令、点击数、归因用户数、新注册人数、新充值人数、新会员人数、充值用户数和订单数；“推广链接”单元格固定两行，每行以“落地页”或“OneLink”为字段名并直接显示对应 URL，同时提供打开、复制操作，过长 URL 以省略号收束且悬停可查看完整地址，历史单变体保留字段名并将缺失值显示为“暂无”。转化指标由后端关联 GoodShort 日报后累计返回，同一口令只展示一份；页面不展示链接类型、批次、状态、失败原因、操作列或充值金额，也不提供重试、修改或重新提交。
