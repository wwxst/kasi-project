...
`/workspace/promotion-links` 菜单显示为“推广任务”，按单条推广记录只读展示创建时间、推广名称、短剧、媒体平台、口令、推广链接、点击数、归因用户数、新注册人数、新充值人数、新会员人数、充值用户数和订单数。转化指标由后端关联 GoodShort 日报后累计返回；页面不展示链接类型、批次、状态、失败原因、操作列或充值金额，也不提供重试、修改或重新提交。
