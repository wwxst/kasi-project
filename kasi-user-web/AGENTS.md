...
- “短剧推广”在当前页面打开所选短剧详情，用户二选一生成落地页或 OneLink，每个媒体平台只创建一条所选类型的口令/链接，整批生成成功后才进入推广任务页并提示成功；部分失败时留在当前创建弹窗并提示重新发起推广。推广任务主列表按单条记录展示创建时间、推广名称、短剧、媒体平台、口令、推广链接和七项累计转化指标，不显示链接类型、批次、状态、失败原因、操作列或充值金额，不提供重试、修改或重新提交。
