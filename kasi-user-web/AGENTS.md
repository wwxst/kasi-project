...
- 个人中心通过工作区 Header 头像菜单进入，使用 `/api/user/auth/me` 展示本人资料；昵称和真实姓名通过 `/api/user/auth/profile` 修改，头像通过 `/api/user/auth/avatar` 上传，手机号、邮箱、用户编号、状态和登录信息保持只读。密码修改先向当前绑定手机号发送并验证 `CHANGE_PASSWORD` 验证码，验证通过后才显示新密码表单，再通过 `/api/user/auth/password` 提交一次性凭证；凭证只保存在页面内存，成功后清除本地会话并返回登录页。
...
- “短剧推广”在当前页面打开所选短剧详情；用户选择媒体平台并发起一次请求，每个媒体平台固定创建落地页和 OneLink 两个变体，整批全部成功后才进入推广任务页并提示成功，部分失败时留在当前创建弹窗并提示重新发起推广。推广任务主列表按口令一行展示创建时间、推广名称、短剧、媒体平台、口令、独立的“OneLink”和“落地页”两列及七项累计转化指标；两个链接列宽均为 320px，完整 URL 在列内自然换行且不省略，并继续用于打开、悬停提示和复制，历史单变体在对应列显示“暂无”。页面不显示链接类型、批次、状态、失败原因、操作列或充值金额，不提供重试、修改或重新提交。
- 本人订单页按 `paidAt` 月份分页查询后端已归因订单，支持导出所选月份 CSV；前端只展示甲方订单号、`PAID/REFUNDED` 支付状态、支付时间、甲方推广口令 `searchCode` 和本人的佣金快照，不展示完整订单金额或内部佣金状态，不重算分佣、不修改费率。当前不提供独立佣金页入口。
