...
- “短剧推广”在当前页面打开所选短剧详情；用户选择媒体平台并发起一次请求，每个媒体平台固定创建落地页和 OneLink 两个变体，整批全部成功后才进入推广任务页并提示成功，部分失败时留在当前创建弹窗并提示重新发起推广。推广任务主列表按口令一行展示创建时间、推广名称、短剧、媒体平台、口令、同一单元格内分两行显示的两个推广 URL 和七项累计转化指标；两行分别保留“落地页”和“OneLink”字段名，长 URL 可省略展示但必须保留完整地址用于打开、悬停提示和复制，历史单变体保留字段名并将缺失值显示为“暂无”。页面不显示链接类型、批次、状态、失败原因、操作列或充值金额，不提供重试、修改或重新提交。
