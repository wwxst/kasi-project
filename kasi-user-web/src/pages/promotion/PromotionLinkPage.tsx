...
export function PromotionLinkPage() {
  ...
  return (
    <section
      className="promotion-link-page"
      aria-labelledby="promotion-link-page-title"
    >
      <div className="promotion-link-breadcrumb">
        推广管理 / <span>创建推广</span>
      </div>
      <div className="promotion-link-heading">
        <p className="page-eyebrow">CREATE PROMOTION</p>
        <h1 id="promotion-link-page-title">创建推广</h1>
      ...
      >
        ...
        <Table
              ...
              cell: ({ row }) => (
                <>
                  {row.commissionScopes?.map((scope) => (
                    <Tag key={scope}>{scope === 'ORDER' ? '订单' : '广告'}</Tag>
                  ))}
                </>
              ),
            },
          ...
          empty={<div className="promotion-library-empty">暂无可推广短剧</div>}
        />
      </section>
