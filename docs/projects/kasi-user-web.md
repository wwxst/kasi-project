...
个人中心通过工作区 Header 头像菜单进入，使用 `/api/user/auth/me` 展示本人资料。用户名下方只读显示学员类型（`0=基础用户`、`1=基础学员`），不属于编辑资料字段。基本信息支持编辑昵称、真实姓名、微信号、手机号码和电子邮箱，并通过 `/api/user/auth/profile` 保存；手机号或邮箱变更会使旧会话失效，需要重新登录。页面不展示实名认证入口，基本信息和安全设置使用椭圆形白色切换按钮，安全设置内容不再嵌套卡片。头像无蓝色外圈。密码修改先验证当前绑定手机号的 `CHANGE_PASSWORD` 短信验证码，验证成功后才显示新密码表单；一次性改密凭证只保存在页面内存，成功后清除旧会话和查询缓存并返回登录页，未绑定手机号时提示先绑定。
