...
- [x] 编写 V18 从 V17 数据库迁移后默认值为 `PUBLISHED`、历史 DRAFT 按远端状态转换的失败测试。
- [x] 运行聚焦测试确认测试先失败。
- [x] 实现按 MySQL/H2 数据库类型设置默认值和历史数据转换。
- [x] 重新运行迁移测试，确认通过。
...
- [x] 添加新建在线、新建下架、已有上架遇甲方下架、甲方恢复在线不自动上架四个场景。
- [x] 运行聚焦测试确认缺少同步状态更新时失败。
- [x] 修改 upsert：新记录按甲方状态决定本地状态；重复记录仅在甲方非在线且我方已上架时置为 OFFLINE。
- [x] 运行聚焦测试确认通过。
...
- [x] 添加远端状态映射测试：`1` 显示“在线”，非 `1` 显示“已下架”，空值显示“未知”。
- [x] 实现映射并保留原始状态可在详情中查看。
- [x] 运行管理端页面测试。

### Task 4: 完整验证与文档

- [x] 运行后端完整测试、管理端完整测试、lint、typecheck、build。
- [x] 更新根架构文档和后端 README 的当前行为描述。
- [x] 运行 `git diff --check`，提交并推送根仓库 `master`。
