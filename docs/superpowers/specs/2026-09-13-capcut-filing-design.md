...
本期只实现 CapCut 项目的人工账号报白闭环：推广用户提交账号资料，管理员在本系统中维护当前报白状态，推广用户查看结果。

本期报白与现有 GoodShort 媒体账号报白分开存储、分开提供接口和页面。两套业务只统一状态名称和页面展示文案，不复用 GoodShort 的 Provider、连接、报白任务或 Worker。

## 2. 已确认规则

- CapCut 报白为人工流程，不调用 CapCut 官方报白 API。
- 新提交记录直接进入 `PENDING`，不存在 `NOT_SUBMITTED`。
...
| 状态 | 用户端文案 | 含义 |
| --- | --- | --- |
| `PENDING` | 审核中 | 用户已提交，等待管理员处理 |
| `APPROVED` | 已加白 | 当前报白结果为通过 |
| `REJECTED` | 未通过 | 当前报白结果为未通过 |
...
- 不在本期把 CapCut 报白记录关联到 `promotion_project`。现有 `promotion_project_type` 的 `CPA/CPM/CPS` 只表示计费分类，不能唯一标识 CapCut 项目。

## 4. 数据结构

新增独立表 `promotion_capcut_filing_account`：
...
- 删除推广用户前必须检查该用户是否存在 CapCut 报白记录；存在时禁止物理删除，避免产生孤儿数据。
...
后端必须按 `accountType` 校验条件字段，不能只依赖前端隐藏无关输入。创建成功后状态固定为 `PENDING`，客户端不能指定状态。
...
页面下方显示本人提交记录及三种状态。不显示“关联 TikTok 账号”“绑定 CapCut 账号”等控件，也不要求先提交另一种账号。
...
页面提供分页列表、用户编号/账号类型/状态/关键词筛选、状态更新和删除操作。页面不显示审核人、拒绝原因、报白方式、外部报备编号、失败原因、重试按钮或任务信息。

## 8. 错误行为

- CapCut UID 已存在：返回明确的 UID 重复业务错误。
- 条件字段不符合账号类型：返回参数校验错误。
- 记录不存在：返回 CapCut 报白记录不存在业务错误。
- 用户越权查询：用户接口通过 `user_id` 限定，不返回其他用户记录。
- 管理员提交 `PENDING` 或非法状态转换：返回状态操作不允许业务错误。
- 删除不存在记录：返回 CapCut 报白记录不存在业务错误。
...
1. 新增数据库迁移、开发重建结构、后端 Entity/Mapper/DTO/VO 和针对性测试。
2. 实现用户查询、提交接口和推广用户删除保护。
3. 实现管理员分页、状态更新和删除接口。
4. 实现用户端独立 CapCut 报白页面。
5. 实现管理端独立 CapCut 报白页面。
6. 完成三端 Gate 后更新当前架构和缺口文档。

## 11. 与旧草案的关系

本设计只处理 CapCut 人工报白。确认后，它将替代 `2026-09-09-capcut-filing-and-order-attribution-design.md` 中的报白部分；旧草案中的订单归因内容继续保持未确认、未实施，不能随本设计一起进入开发。

## 12. 待确认项

以下事项尚未从当前需求中得到最终口径：

1. 本期是否同时包含独立 TikTok 账号提交；本稿按“包含 TikTok 和 CapCut 两类独立账号”编写。
2. `account_region` 是否固定为 `US / EU / ROW`；本稿示例按该枚举编写，但实现前需要确认。
3. 账号资料填错时是否允许管理员删除任意状态的本地记录；本稿按“允许删除，用户重新提交”编写。
4. 本期是否使用独立菜单且暂不关联具体 `promotion_project`；本稿按独立菜单和独立业务表编写，不通过项目名称或 `CPA/CPM/CPS` 类型识别 CapCut。

三状态、无审核人、无拒绝原因、无官方 API、无订单和结算已经确认；其他字段、接口和页面行为仍以本稿为评审建议，整份设计经确认后再实施。
