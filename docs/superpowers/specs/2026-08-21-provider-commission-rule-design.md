...
当前仓库已经完成短剧平台接入、媒体账号报备和 GoodShort 短剧目录同步。下一阶段先交付平台分佣规则，为推广链接生成和订单预计佣金计算提供稳定的费率来源。
...
新增 Flyway 迁移 `V8__provider_commission_rule.sql`，建立 `provider_commission_rule` 表。
...
管理员 API 使用百分比输入和输出，`30` 表示 `30%`。五项费率都必须明确提交，范围为 `0..100`，最多保留四位百分比小数。渠道费率、甲方手续费率和我方手续费率可由前端默认填 `0`，但请求中仍必须出现。
...
- 运行短剧模块聚焦测试、完整 `mvnw.cmd test`、Java 25 编译检查和 `git diff --check`。

## 13. 文档同步

实现完成后同步更新 `README.md`、`AGENTS.md`、多平台路线图和总设计，将平台级分佣规则标记为已实现；推广链接、订单、费率快照和转化分析继续明确标记为未实现。
