...
src/
  main/
    ...
    resources/
      ...
      db/
        kasi_promotion.sql                  # 空库完整初始化、默认数据和默认超级管理员
...
| 数据库初始化 | 应用不自动建表或升级；首次启动前手动对空库执行 `src/main/resources/db/kasi_promotion.sql` |
...
剧集播放和素材下载还需要配置 GoodShort 资源域名白名单并安装 FFmpeg：

```powershell
$env:GOODSHORT_MEDIA_HOSTS = 'cdn.example.com,media.example.com'
$env:APP_FFMPEG_PATH = 'ffmpeg'
$env:APP_DOWNLOAD_DIR = 'E:\kasi-data\downloads'
$env:APP_DOWNLOAD_CONNECT_TIMEOUT = '10s'
$env:APP_DOWNLOAD_READ_TIMEOUT = '30s'
```

`GOODSHORT_MEDIA_HOSTS` 使用逗号分隔，只允许对应域名及其子域名；不得配置 localhost 或内网 IP。下载目录默认 `./var/downloads`，连接超时默认 10 秒、单次读取超时默认 30 秒，文件默认 24 小时过期。生产环境应使用独立持久盘，并确保应用进程只有该目录的必要读写权限。

首次启动前，先连接到已经创建好的空 schema，并在 mysql 客户端中执行唯一初始化 SQL：
...
密码在数据库中保存为 BCrypt 哈希。首次登录后应立即通过 `PUT /api/admin/auth/password` 修改默认密码。项目当前仍处于可重建数据库的开发阶段；已有开发数据库不会被兼容升级，应删除并创建空库后重新执行初始化 SQL。
...
唯一初始化 SQL 定义当前数据库持久表，验证码和密码重置 Token 等临时数据由 Redis 管理：
...
| MySQL | `drama_download_task` | 用户素材下载任务、进度、ZIP 路径和过期时间 | user_id, drama_id, status, content_ids_json, completed_count, file_path, expires_at |
...
| Redis | `drama:free-content:*` | GoodShort 免费剧集资源短缓存 | 默认 5 分钟 TTL，不保存 PID 或 API KEY |

`kasi_promotion.sql` 按当前完整结构一次建库，并在建表后直接插入 `admin` 超级管理员和一个启用的初始推广用户。管理员固定写入 `status=1`、`is_super_admin=1`；推广用户使用邮箱登录，密码和管理员密码均只以 BCrypt 哈希保存。该文件只用于空库手动初始化，不会在应用启动时自动执行，也不植入任何平台接入密钥。
...
推广用户可查询已上架短剧、查看剧集并生成 GoodShort 推广链接/口令。Chrome 使用 `hls.js` 播放 HLS；素材下载创建后端任务，HLS 通过 FFmpeg 合并为 MP4，多集生成 ZIP，任务支持进度、失败状态、三次下载重试、一次失效地址刷新和过期清理；普通媒体下载受连接超时和单次读取超时限制。当前只处理 GoodShort 免费内容；付费剧集授权、正式账单、钱包、提现和转化分析仍未实现。
...
用户端短剧详情通过 `GET /api/user/promotion/dramas/{id}/free-content` 获取 GoodShort `/open/book/freeContent` 返回的免费剧集资源地址。资源 Redis TTL 默认 5 分钟；播放器或下载器发现地址失效时通过 `refresh=true` 最多强制刷新一次。创建和读取下载任务的三个用户接口为：
...
真实平台烟雾测试使用 `scripts/dev/smoke-goodshort-free-content.ps1`。脚本要求 `GOODSHORT_BASE_URL`、`GOODSHORT_PARTNER_ID`、`GOODSHORT_API_KEY` 和 `DRAMA_EXTERNAL_ID`，只检查变量是否存在，不打印变量值。当前仓库未保存任何联调凭据。
...
| GET | `/api/user/promotion/orders/monthly` | USER | 查询本人月度订单金额及计算/冲销/净佣金汇总 |
| GET | `/api/user/promotion/orders/export.csv` | USER | 导出本人指定月份订单 CSV |
...
| 测试类 | 说明 |
|--------|------|
| `DefaultSuperAdminMigrationTest` | 直接执行唯一初始化 SQL，验证初始账号、权限字段和 BCrypt 密码 |
...
| `HistoricalCompatibilityStructureTest` | 认证接口、Mapper 和错误码不保留无调用的历史兼容残留 |
...
3. ✅ 使用唯一 `kasi_promotion.sql` 手动初始化空库，应用不自动迁移数据库。
...
用户通过 `/api/user/promotion/links` 提交 `providerId`、`dramaId`、`mediaTypes` 和 `requestKey`。每个选中的 TikTok、YouTube、Facebook 或 Instagram 平台生成一条 `LANDING` 和一条 `ONELINK`，同批次共享 `batchNo`，每条记录使用独立 `trackingNo/customParams`，不绑定媒体账号或报白状态。部分变体失败时保留成功结果，失败记录可重试。
