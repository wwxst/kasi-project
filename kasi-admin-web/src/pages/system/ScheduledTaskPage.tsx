...
export function ScheduledTaskPage() {
  ...
  const requestFor = (
    ...
    ...(task.cycleType === 'INTERVAL_HOURS' ||
    task.cycleType === 'INTERVAL_DAYS'
      ? {
  ...
  const handleToggle = async (task: ScheduledTask, enabled: boolean) => {
    ...
    try {
      replaceTask(
        await updateScheduledTask(task.taskCode, requestFor(task, enabled)),
      )
  ...
  const handleCycleChange = (value: ScheduledTaskCycleType) => {
    ...
    form.setFieldsValue({
      cycleType: value,
      intervalValue:
        value === 'INTERVAL_HOURS' || value === 'INTERVAL_DAYS'
          ? current && current <= 24
            ? current
            : 1
          : (current ?? 1),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const request: UpdateScheduledTaskRequest = {
        ...
        ...(values.cycleType === 'INTERVAL_HOURS' ||
        values.cycleType === 'INTERVAL_DAYS'
          ? {
          ...
          : {}),
        timeOfDay:
          values.cycleType === 'INTERVAL_SECONDS' ||
          values.cycleType === 'INTERVAL_MINUTES'
            ? undefined
            : timeValue(values),
        ...
        >
          <Form.Item label="执行周期" required>
            ...
            <div className="scheduled-task-page__cycle-help">
              {formatFormCycle(
                { ...watchedValues, intervalHoursPart, intervalMinutesPart },
                cycleType,
                intervalValue,
              )}
            </div>
            <Form.Item name="cycleType" hidden>
              <Input aria-label="执行周期" />
            </Form.Item>
          </Form.Item>
          <Form.Item
            label="任务说明"
...
function IntervalFields({ cycleType }: { cycleType: ScheduledTaskCycleType }) {
  ...
  return (
    <>
      ...
      >
        <UnitNumber
          max={cycleType === 'INTERVAL_DAYS' ? 365 : 1440}
          unit={unit}
          ariaLabel="执行周期数值"
        />
      </Form.Item>
...
function CalendarFields({ cycleType }: { cycleType: ScheduledTaskCycleType }) {
  if (cycleType === 'INTERVAL_SECONDS' || cycleType === 'INTERVAL_MINUTES')
    return null
  return (
    <>
      {cycleType === 'WEEKLY' ? (
        <>
          <span>星期</span>
          <Form.Item name="dayOfWeek" noStyle rules={[{ required: true }]}>
            <Select
              className="scheduled-task-page__weekday"
              options={[1, 2, 3, 4, 5, 6, 7].map((value) => ({
                value,
                label: `周${['一', '二', '三', '四', '五', '六', '日'][value - 1]}`,
              }))}
              aria-label="星期"
            />
          </Form.Item>
      ...
      {cycleType === 'MONTHLY' || cycleType === 'YEARLY' ? (
        <>
          <span>日期</span>
          <NumberField name="dayOfMonth" unit="日" max={31} label="日期" />
        </>
      ) : null}
      {cycleType === 'YEARLY' ? (
        <>
          <span>月份</span>
          <NumberField name="monthOfYear" unit="月" max={12} label="月份" />
        </>
      ) : null}
      <span>执行时间</span>
...
function NumberField({
  ...
  return (
    <Form.Item
      name={name}
      noStyle
      rules={[{ required: true }, { type: 'number', min: 0, max }]}
    >
      <UnitNumber max={max} unit={unit} ariaLabel={label} />
    </Form.Item>
  )
}
...
function UnitNumber({
  ...
  return (
    <Space.Compact className="scheduled-task-page__unit-number">
      <InputNumber
        min={0}
        max={max}
        value={value}
      ...
      />
      <span className="scheduled-task-page__unit">{unit}</span>
    </Space.Compact>
  )
}

function formatFormCycle(
  values: Partial<FormValues>,
  type: ScheduledTaskCycleType,
  value?: number,
) {
  if (type === 'INTERVAL_SECONDS') return `每隔${value ?? 0}秒执行一次`
  if (type === 'INTERVAL_MINUTES') return `每隔${value ?? 0}分钟执行一次`
  if (type === 'INTERVAL_HOURS')
    return `每隔${value ?? 0}小时${values.intervalMinutesPart ?? 0}分钟执行一次`
  if (type === 'INTERVAL_DAYS')
    return `每隔${value ?? 0}天${values.intervalHoursPart ?? 0}小时${values.intervalMinutesPart ?? 0}分钟执行一次`
  ...
  if (type === 'WEEKLY')
    return `每星期${values.dayOfWeek ?? ''} ${time}执行一次`
  if (type === 'MONTHLY')
    return `每月${values.dayOfMonth ?? ''}日 ${time}执行一次`
  return `每年${values.monthOfYear ?? ''}月${values.dayOfMonth ?? ''}日 ${time}执行一次`
}

function formatCycle(task: ScheduledTask) {
  const parts = timeParts(task.timeOfDay)
  return formatFormCycle(
    { ...task, ...parts },
    task.cycleType ?? 'INTERVAL_MINUTES',
    task.intervalValue ?? task.intervalMinutes,
  )
}
