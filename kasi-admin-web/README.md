...
当前已完成前端基础工程、管理员登录入口、登录 API 对接、会话保存、受保护路由和管理后台布局。后台壳层包含固定顶栏、导航搜索、语言与通知入口、深浅主题、布局设置、全屏控制、账户菜单、可折叠桌面侧栏、移动端导航抽屉和面包屑。
