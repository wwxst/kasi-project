...
- 媒体账号用户 API 位于 `/api/user/promotion/media-accounts`，管理员 API 位于 `/api/admin/promotion/media-accounts`；推广用户创建或编辑媒体账号时，媒体平台、账号 ID、账号名称和账号主页链接均为必填，主页链接必须使用 HTTPS。API 报备在本地事务提交后立即调用 `/filing/report`，成功只表示进入审核中；上报失败保存错误且不自动重试，用户端无重试入口，管理端仅允许从未成功提交且存在提交错误的记录重新提交。后台 Worker 只查询已成功提交且仍在审核中的记录：甲方状态 `0` 继续查询，`1` 已加白和 `2` 已拒绝均停止。页面统一显示待提交、提交失败、审核中、已加白、已拒绝；账号名称和主页链接变化不重新报备，只有媒体平台或账号 ID 实际变化才重新上报。已加白后锁定身份字段，响应不暴露平台连接 ID、PID、密钥或任务租约字段。
