...
- 推广链接生成的 GoodShort HTTP 调用必须在数据库事务之外，并按 `pid + bookId + customParams + codeMedia` 保证至少 2 秒间隔；`shareUrlType` 不参与口令唯一性，同一身份的 `LANDING/ONELINK` 两条记录共享 `external_code` 并各自保存 `share_url`。仅 `status=20005` 且 `data.code/shareUrl` 完整时按甲方“已生成”结果同步，其他非零状态继续失败。`PENDING`、`SUCCESS`、`FAILED` 状态分别通过独立短事务持久化。创建请求的 `mediaTypes` 不得重复；每个媒体固定生成 `LANDING` 和 `ONELINK`，同一 `requestKey` 不得改变平台、短剧、媒体集合或完整双变体集合。订单 upsert 使用 `READ_COMMITTED`，遇到 `(connection_id, external_order_id)` 唯一键并发冲突时回读已有订单，不重写同步流程；不得改回会对不存在行产生 gap lock 竞争的默认 `REPEATABLE_READ`。
- 管理后台 Dashboard 当前只显示当前管理员欢迎语，不展示静态 Demo 卡片；侧边栏、品牌链接、搜索回车和兜底路由统一使用 `/user-management`，真实数据大屏仍未实现。
- `sys_admin_user`、`promotion_user` 和媒体账号表均不保留 `deleted_at`。用户端不能删除媒体账号；管理端可不受报白状态、方式、动作或租约限制地单条物理删除媒体账号，并在同一事务中先删除该账号的全部报白记录。删除只影响本系统记录，不会删除甲方记录。
- 媒体账号用户 API 位于 `/api/user/promotion/media-accounts`，管理员 API 位于 `/api/admin/promotion/media-accounts`；媒体账号创建后两端均不可编辑，同一媒体平台和账号 ID 全局唯一。平台接入按 Facebook、TikTok、YouTube、Instagram 配置 `API` 或 `MANUAL` 报白；已有账号切换方式时保留终态和远端证据，已发出但尚未收敛的提交禁止切换。报白持久状态固定为 `NOT_SUBMITTED`、`PENDING`、`APPROVED`、`REJECTED`、`SUBMIT_FAILED`：API 任务由 Worker 分批处理到期的 `SUBMIT` 与 `QUERY`，每次领取使用独立租约 token，提交结果不确定时停止自动重报并由管理员确认甲方已收到或未收到；MANUAL 记录不进入 Worker，管理员按确认矩阵更新状态。用户端将 `SUBMIT_FAILED` 投影为 `PENDING` 并隐藏提交错误，只展示四种业务状态；管理端展示真实五状态、报白方式、错误、人工更新时间/操作人和可用操作。响应不暴露平台连接 ID、PID、密钥或任务租约字段。
- 短剧目录管理员 API 位于 `/api/admin/drama/catalog`；普通管理员和超级管理员均可分页查询、查看详情、触发同步、查询同步状态、修改本地上下架和维护短剧推广元数据（`PUT /api/admin/drama/catalog/{id}/promotion-metadata`）。手动同步语言留空时按配置展开 GoodShort 全部 13 种支持语言并逐语言创建任务，固定全量和增量任务使用同一完整语言集合；同步默认每 5 分钟兜底处理到期任务，支持提交后立即唤醒、断点续跑、过期租约接管和同连接/语言跨 FULL、INCREMENTAL 互斥。新同步的甲方在线短剧默认 `PUBLISHED`，甲方非在线时同步为 `OFFLINE`，已有已上架短剧遇甲方下架时自动下架。单语言全量完整成功后，本轮未返回的历史短剧标记为 `MISSING / OFFLINE`，失败或未完成时不批量下架；不物理删除短剧、剧集或推广元数据。短剧重新出现时恢复甲方真实远端状态和 `last_seen_at`，本地仍保持 `OFFLINE`，由管理员手动决定是否上架。目录同步会为新增、`remoteUpdatedAt` 变化或本地缺少 `content_url` 的短剧自动排队免费剧集同步。目录列表和详情同时返回原始 `language` 与中文 `languageLabel`，管理端将 `MISSING` 显示为“全量未返回”。
- 用户端短剧 API `/api/user/promotion/dramas` 只返回本地已上架且甲方在线的短剧，列表按甲方 `remoteCreatedAt` 发布时间倒序（`remote_created_at DESC, id DESC`）分页，不使用本地 `createdAt` 代替发布时间；管理端和用户端共享 `GET /api/drama/languages` 语言选项接口，该接口按实际生效的 `DramaSyncProperties.languages` 返回 `{value,label}`，筛选提交原始语言码。
...
- 推广链接、转化日报和订单级 CPS 最小闭环已实现：用户通过 `/api/user/promotion/links` 生成 GoodShort 链接/口令，`requestKey` 约束同一请求内容并保存内部 `trackingNo`；只有整批生成成功时前端才跳转并提示成功。甲方日报只有口令维度，因此用户任务查询按 `connection + drama + externalCode`（等价于 `pid + customParams + bookId + code`）聚合成一行：`landingUrl`/`oneLinkUrl` 分别取该口令的 LANDING 与 ONELINK 地址，七项累计次数/人数只返回一份，分页 `total` 与 `offset/limit` 同样以口令为单位，禁止按变体重复统计。跨媒体共用同一口令时（`COUNT(DISTINCT media_type) > 1`）媒体无法确定，返回 `analyticsConflict=true` 且媒体与指标为 `null`，不得用 `MIN(id)`/`LIMIT 1` 静默选择某个变体或媒体。管理端 `GET /api/admin/promotion/links` 使用同一口令级语义：`trackingNo` 命中组内任一原始链接后返回完整口令组，不裁掉另一变体或改变冲突判断；响应不再逐行返回没有唯一取值的 `trackingNo`、`linkVariant`、`shareUrl`。两个查询都不返回 `orderAmount`。`GOODSHORT_ANALYTICAL_REPORT_SYNC` 每日同步前一天日报，并核对 PID、bookId 和 `customParams -> user_no`。生成链接/口令不要求平台预先配置分佣规则，分佣规则只在已支付订单计算佣金时读取。`GOODSHORT_ORDER_SYNC` 每分钟自动同步最近 3 天，管理员仍可通过 `POST /api/admin/promotion/orders/sync` 手动补拉指定范围，订单以 `(connection_id, external_order_id)` 幂等，按 `pid + customParams + bookId + searchCode` 匹配 code 后归因到用户和短剧并保存原始 JSON、甲方 `searchCode` 和五费率快照；LANDING/ONELINK 共用 code 时不再通过任意变体写入 `promotion_link_id` 或 `tracking_no`。本地订单状态只有 `PAID`/`REFUNDED`，未支付或未知甲方状态不落库。管理员可查询/CSV 导出完整核对字段；用户只读取本人订单规定字段。退款保留原佣金并标记 `REVERSED`。正式账单、钱包和提现仍未实现。
- 推广任务壳已删除；用户端以真实 `PromotionLink` 为推广入口。
- 定时任务管理 API 位于 `/api/admin/system/scheduled-tasks`；固定任务 `GOODSHORT_DRAMA_FULL_SYNC` 默认每天 `03:00` 为配置语言入队完整目录快照，`GOODSHORT_DRAMA_INCREMENTAL_SYNC` 默认每 60 分钟入队，`GOODSHORT_DRAMA_CONTENT_SYNC` 默认每 1 分钟处理免费剧集队列，`GOODSHORT_ORDER_SYNC` 默认每 1 分钟同步最近 3 天。增量任务仍要求对应语言存在成功全量基线。周期支持 `INTERVAL_SECONDS/MINUTES/HOURS/DAYS`、`DAILY`、`WEEKLY`、`MONTHLY`、`YEARLY`，`INTERVAL_HOURS` 使用小时数和 `interval_minutes_part` 分钟余量，`INTERVAL_DAYS` 使用天数、`interval_hours_part` 小时余量和 `interval_minutes_part` 分钟余量；日历型周期同时保存执行时间及对应星期/日期字段。每分钟调度器扫描并执行到期任务，订单、目录和免费剧集任务复用同一分发器和数据库租约；普通管理员只读，超级管理员可编辑周期、说明和启停状态。
...
- 推广链接当前按批次和媒体平台生成，不绑定媒体账号或报白状态；一次请求为每个选中媒体固定生成 `LANDING`、`ONELINK` 两条记录。同一 `pid + bookId + customParams + codeMedia` 的两个变体共享 GoodShort code，各自保存对应 URL；内部 `tracking_no` 与发送甲方的稳定 `customParams=user_no` 分工明确。只有全部媒体的两个变体均成功时用户端才进入任务页；任务页按口令一行展示两个链接和一份转化指标，不展示状态、失败原因或操作，也不提供重试、修改或重新提交。用户重新发起推广时创建新任务。
