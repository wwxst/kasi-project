...
> 历史设计：当前创建接口使用 `mediaTypes` 和可选 `linkVariant`，不再绑定媒体账号；用户在 `LANDING` 与 `ONELINK` 中二选一，每个媒体平台只生成一条所选类型记录。当前行为以仓库 README 和 AGENTS.md 为准。
...
失败时不创建成功链接，不绕过报备或上下架校验。分佣规则不属于链接生成的前置条件，只在已支付订单计算佣金时读取。历史已生成链接不因短剧后续下架或媒体账号停用而删除。
...
验收标准：同一个 `requestKey` 重试不会产生第二条记录；未加白或未上架时不能调用第三方；未配置分佣规则仍可生成链接/口令；成功记录保存 `trackingNo`、`code`、`shareUrl` 和 `customParams`；所有测试和完整回归通过。
