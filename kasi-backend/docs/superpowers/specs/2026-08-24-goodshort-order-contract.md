...
| `payStatus` | integer | `rawStatus` / `status` | 0 未支付、1 已支付、3 退款；本地只落库已支付和已退款 |
...
适配器可识别甲方原始值 `0`、`1`、`3` 并保留 `rawStatus`；同步服务分别映射为本地 `UNPAID`、`PAID`、`REFUNDED`。其他未知值映射为 `UNKNOWN`，四种状态均写入 `promotion_order`，未知状态不计算佣金。
...
用户端订单接口和 CSV 只返回 `externalOrderId`、连接币种、`UNPAID/PAID/REFUNDED` 支付状态、`paidAt`、`trackingNo` 和当前用户的 `commissionAmount`，过滤 `UNKNOWN`。本地订单主键、`orderAmount`/`orderAmountMinor`、原始 JSON、归因字段、费率快照和内部 `commissionStatus` 仅供服务端分佣或管理员核对，不对推广用户暴露。
