-- Kasi Promotion 空数据库初始化脚本
-- 仅用于空库首次建表，不包含历史数据库升级或数据迁移逻辑。
--
-- 使用前请先手动创建数据库：
...
(
    ...
    KEY `idx_promotion_order_attribution` (`attribution_status`, `paid_at`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='GoodShort短剧推广订单及佣金快照';

-- 短剧素材下载任务
CREATE TABLE `drama_download_task` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '下载任务ID',
    `user_id` BIGINT UNSIGNED NOT NULL COMMENT '推广用户内部ID',
    `drama_id` BIGINT UNSIGNED NOT NULL COMMENT '短剧ID',
    `status` VARCHAR(16) NOT NULL DEFAULT 'PENDING' COMMENT 'PENDING排队 RUNNING执行中 SUCCESS成功 FAILED失败',
    `content_ids_json` TEXT NOT NULL COMMENT '剧集ID JSON数组',
    `file_path` VARCHAR(1024) DEFAULT NULL COMMENT '服务端生成的ZIP绝对路径',
    `file_name` VARCHAR(255) DEFAULT NULL COMMENT '下载文件名',
    `total_count` INT NOT NULL DEFAULT 0 COMMENT '总剧集数',
    `completed_count` INT NOT NULL DEFAULT 0 COMMENT '已完成剧集数',
    `error_message` VARCHAR(512) DEFAULT NULL COMMENT '脱敏失败信息',
    `expires_at` DATETIME NOT NULL COMMENT '文件过期时间',
    PRIMARY KEY (`id`),
    KEY `idx_drama_download_status_expires` (`status`, `expires_at`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='短剧素材下载任务';
