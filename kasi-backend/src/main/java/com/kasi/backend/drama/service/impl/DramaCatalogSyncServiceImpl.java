...
public class DramaCatalogSyncServiceImpl implements DramaCatalogSyncService {
    ...
    private final DramaSyncDisplayRunMapper displayRunMapper;
    ...
    private final com.kasi.backend.drama.service.DramaSyncDisplayRunService displayRunService;
    ...
    public DramaCatalogSyncServiceImpl(ProviderSyncCheckpointMapper checkpointMapper,
                                       DramaSyncDisplayRunMapper displayRunMapper,
                                       ...
                                       com.kasi.backend.drama.service.DramaSyncDisplayRunService displayRunService,
        ...
        this.displayRunMapper = displayRunMapper;
    ...
    public List<DramaSyncTaskVO> requestSync(Long providerId, DramaSyncType requestedType, List<String> languages) {
        ...
        ProviderRuntimeConnection runtime = runtimeService.resolve(providerId, requestedCapability);
        ...
        DramaSyncDisplayRun displayRun = displayRunService.createRun(providerId, null, DramaSyncDomain.CATALOG,
                ...
                SyncTriggerSource.MANUAL, now);
        DramaSyncDisplayRun contentRun = displayRunService.createRun(providerId, displayRun.getId(),
                DramaSyncDomain.CONTENT, DramaSyncTaskType.CATALOG_AUTO, SyncTriggerSource.MANUAL, now);
        LinkedHashSet<DramaSyncType> effectiveTypes = new LinkedHashSet<>();
        for (String language : normalizeLanguages(languages)) {
            ...
            effectiveTypes.add(effectiveType);
    ...
    private void processClaimed(ProviderSyncCheckpoint checkpoint) {
        ...
        try {
            ...
            if (!(runtime.adapter() instanceof DramaCatalogProviderAdapter adapter)) {
                markConnectionNotReady(checkpoint, now);
                return;
            }
            String parentRunId = displayRunMapper.findRunIdByTask(DramaSyncDomain.CATALOG, checkpoint.getId());
            String contentRunId = null;
            if (parentRunId != null) {
                DramaSyncDisplayRun child = displayRunMapper.findChildRun(parentRunId, DramaSyncDomain.CONTENT,
                        DramaSyncTaskType.CATALOG_AUTO);
                contentRunId = child == null ? null : child.getId();
            }
            fetchPages(checkpoint, effectiveType, runtime, adapter, contentRunId);
        } catch (ProviderTransientException exception) {
            markFailure(checkpoint, LocalDateTime.now(clock), "REMOTE_TRANSIENT", safeMessage(exception));
        } catch (ProviderRemoteRejectedException exception) {
            markFailure(checkpoint, LocalDateTime.now(clock), "REMOTE_REJECTED", safeMessage(exception));
        } catch (LeaseLostException ignored) {
            // Another worker owns the task now; this worker must not write any terminal state.
        } catch (RuntimeException exception) {
            try {
                markFailure(checkpoint, LocalDateTime.now(clock), "TASK_ERROR",
    ...
    private void fetchPages(ProviderSyncCheckpoint checkpoint,
        ...
        do {
            ...
            transactionTemplate.executeWithoutResult(status -> {
                PageStats stats = persistPage(runtime.connectionId(), page.items(), contentRunId);
    ...
    private PageStats persistPage(Long connectionId, List<ProviderDramaRecord> records, String contentRunId) {
        if (records == null) {
            throw new IllegalStateException("Provider returned no catalog records");
        }
        int inserted = 0;
        int updated = 0;
        for (ProviderDramaRecord record : records) {
                ...
                if (existing == null
                        || !Objects.equals(existing.getRemoteUpdatedAt(), record.remoteUpdatedAt())
                        || dramaMapper.needsContentSync(stored.getId())) {
                    if (contentRunId == null) {
                        contentSyncService.requestAutomatic(stored.getId());
                    } else {
    ...
    private DramaSyncTaskType displayTaskType(LinkedHashSet<DramaSyncType> types) {
        if (types.size() != 1) return DramaSyncTaskType.MIXED;
        return types.iterator().next() == DramaSyncType.FULL
                ? DramaSyncTaskType.FULL : DramaSyncTaskType.INCREMENTAL;
    }
    ...
    private ProviderSyncCheckpoint ensureCheckpoint(Long connectionId, DramaSyncType syncType, String language) {
        ...
        if (stored == null) {
            throw new IllegalStateException("Inserted catalog checkpoint cannot be reloaded");
        }
        return stored;
    }
    ...
    private List<String> normalizeLanguages(List<String> requested) {
        ...
        if (normalized.isEmpty()) normalized.addAll(DramaSyncProperties.DEFAULT_LANGUAGES);
