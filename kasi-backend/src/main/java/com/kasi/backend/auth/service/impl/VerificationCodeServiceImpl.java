...
public class VerificationCodeServiceImpl implements VerificationCodeService {
    ...
    private static final DefaultRedisScript<Long> VERIFY_CODE_SCRIPT = new DefaultRedisScript<>("""
            ...
            return -1
            """, Long.class);
    private static final DefaultRedisScript<Long> ROLLBACK_SEND_SCRIPT = new DefaultRedisScript<>("""
            redis.call('DEL', KEYS[1], KEYS[2], KEYS[3])
            local count = tonumber(redis.call('GET', KEYS[4]) or '0')
            if count <= 1 then redis.call('DEL', KEYS[4]) else redis.call('DECR', KEYS[4]) end
            return 1
            """, Long.class);
    ...
    private void sendVerificationCode(String target, VerificationScene scene, boolean deliver) {
        ...
        if (acquired == null) {
            throw new AuthStateUnavailableException(
                    new IllegalStateException("Redis 未返回验证码冷却状态"));
        }
        ...
        if (dailyCount == null) {
            throw new AuthStateUnavailableException(
                    new IllegalStateException("Redis 未返回验证码发送次数"));
        }
        if (dailyCount == 1) {
            // 首次递增，设置过期到当天午夜
        ...
        if (deliver) {
            ...
            try {
                verificationCodeSender.send(target, targetType, scene, code);
            } catch (VerificationDeliveryUnavailableException deliveryException) {
                redisTemplate.execute(ROLLBACK_SEND_SCRIPT,
                        List.of(codeKey(target, scene), cooldownKey(target, scene),
                                failKey(target, scene), dailyKey));
    ...
    public boolean verifyCode(String target, VerificationScene scene, String code) {
        ...
        if (Long.valueOf(VERIFY_SUCCESS).equals(result)) {
            return true;
        }
        if (Long.valueOf(VERIFY_TOO_MANY_FAILURES).equals(result)) {
            throw new BusinessException(ErrorCode.VERIFICATION_CODE_ERROR.getCode(), "验证码错误次数过多，请重新发送");
        }
        if (result == null) {
            throw new AuthStateUnavailableException(
                    new IllegalStateException("Redis 未返回验证码校验结果"));
        }
