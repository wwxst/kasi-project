...
public class UserAuthServiceImpl implements UserAuthService {
    ...
    public UserLoginVO login(UserLoginDTO request, String clientIp) {
        ...
        return completeLogin(user, account, clientIp);
    }
    ...
    private UserLoginVO completeLogin(PromotionUser user, String account, String clientIp) {
        // 生成Token
    ...
    @Override
    @Transactional
    public CurrentUserVO updateProfile(Long userId, UpdateUserProfileDTO request) {
    ...
    @Override
    @Transactional
    public CurrentUserVO updateAvatar(Long userId, MultipartFile file) {
        PromotionUser user = promotionUserMapper.findByIdForUpdate(userId);
        if (user == null) {
            throw new BusinessException(ErrorCode.USER_NOT_FOUND);
        }
        String previousAvatarUrl = user.getAvatarUrl();
        String avatarUrl = userAvatarStorageService.store(file);
        try {
            if (promotionUserMapper.updateAvatar(userId, avatarUrl) != 1) {
                throw new IllegalStateException("用户头像更新未生效");
            }
            registerAvatarFileCompletion(avatarUrl, previousAvatarUrl);
            return getCurrentUser(userId);
        } catch (RuntimeException exception) {
            userAvatarStorageService.deleteIfLocal(avatarUrl);
            throw exception;
        }
    }
    ...
    private void registerAvatarFileCompletion(String avatarUrl, String previousAvatarUrl) {
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCommit() {
                userAvatarStorageService.deleteIfLocal(previousAvatarUrl);
            }
