...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(VerificationDeliveryUnavailableException.class)
    public ResponseEntity<ApiResponse<Void>> handleVerificationDeliveryUnavailable(
            VerificationDeliveryUnavailableException e) {
        log.error("验证码发送服务不可用", e);
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(ApiResponse.error(ErrorCode.VERIFICATION_CODE_SEND_FAILED.getCode(),
                        ErrorCode.VERIFICATION_CODE_SEND_FAILED.getMessage()));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleMethodNotSupported(HttpRequestMethodNotSupportedException e) {
        return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
                .body(ApiResponse.error(HttpStatus.METHOD_NOT_ALLOWED.value(), "请求方法不支持"));
    }
