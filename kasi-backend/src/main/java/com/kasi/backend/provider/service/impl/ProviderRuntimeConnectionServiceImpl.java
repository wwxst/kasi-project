...
public class ProviderRuntimeConnectionServiceImpl implements ProviderRuntimeConnectionService {
    ...
    private boolean isConfigurationFailure(BusinessException exception) {
        int code = exception.getCode();
        return code == ErrorCode.PROVIDER_NOT_FOUND.getCode()
                || code == ErrorCode.PROVIDER_CONNECTION_NOT_FOUND.getCode()
                || code == ErrorCode.PROVIDER_CONNECTION_INVALID.getCode()
                || code == ErrorCode.PROVIDER_CREDENTIAL_UNAVAILABLE.getCode()
                || code == ErrorCode.PROVIDER_CAPABILITY_UNSUPPORTED.getCode();
    }
}
