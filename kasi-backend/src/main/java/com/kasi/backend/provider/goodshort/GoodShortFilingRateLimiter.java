...
/** Dedicated throttle for filing submit/query calls (independent from catalog calls). */
final class GoodShortFilingRateLimiter {
    ...
    GoodShortFilingRateLimiter() {
        this(DEFAULT_INTERVAL, System::nanoTime, LockSupport::parkNanos);
    }

    GoodShortFilingRateLimiter(Duration interval, LongSupplier nanoTime, LongConsumer sleeper) {
        if (interval.isNegative() || interval.isZero()) {
            throw new IllegalArgumentException("Filing rate-limit interval must be positive");
        }
