...
final class GoodShortCatalogRateLimiter {
    private static final Duration DEFAULT_INTERVAL = Duration.ofMillis(610);
    ...
    GoodShortCatalogRateLimiter() {
        this(DEFAULT_INTERVAL, System::nanoTime, LockSupport::parkNanos);
    }

    GoodShortCatalogRateLimiter(Duration interval, LongSupplier nanoTime, LongConsumer sleeper) {
        if (interval.isNegative() || interval.isZero()) {
            throw new IllegalArgumentException("Catalog rate-limit interval must be positive");
        }
