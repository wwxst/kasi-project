...
class ScheduledTaskDispatchServiceTest {
    ...
    void setUp() {
        ...
        service = new ScheduledTaskDispatchServiceImpl(
                taskMapper, providerMapper, syncService, contentSyncService, transactionManager,
    ...
    void dueGoodShortTaskIsDispatchedAndAdvanced() {
        ...
        verify(syncService).requestScheduledIncremental(7L, DramaSyncProperties.DEFAULT_LANGUAGES);
    ...
    void dueDramaContentTaskIsDispatchedAndAdvanced() {
        ...
        when(taskMapper.findDue(NOW, 10)).thenReturn(List.of(task));
        when(taskMapper.claimLease(ScheduledTaskCode.GOODSHORT_DRAMA_CONTENT_SYNC, "scheduled-worker-test", NOW,
                NOW.plusMinutes(2))).thenReturn(1);

        service.processDueBatch();

        verify(contentSyncService).processDueBatch();
        verify(taskMapper).completeRun(ScheduledTaskCode.GOODSHORT_DRAMA_CONTENT_SYNC,
                "scheduled-worker-test", NOW.plusMinutes(1));
    }
    ...
    private SystemScheduledTask scheduledContentTask() {
        SystemScheduledTask task = scheduledTask();
        task.setTaskCode(ScheduledTaskCode.GOODSHORT_DRAMA_CONTENT_SYNC);
        task.setIntervalValue(1);
        task.setCycleType(com.kasi.backend.scheduledtask.enums.ScheduledTaskCycleType.INTERVAL_MINUTES);
        return task;
    }
