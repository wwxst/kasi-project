    ...
    void ordinaryAdminCanReadButCannotUpdate() throws Exception {
        ...
        mockMvc.perform(get(BASE_PATH)
                ...
                .andExpect(jsonPath("$.data[?(@.taskCode == 'GOODSHORT_DRAMA_INCREMENTAL_SYNC')].nextRunAt")
                        .isEmpty())
                .andExpect(jsonPath("$.data[?(@.taskCode == 'GOODSHORT_DRAMA_INCREMENTAL_SYNC')].leaseOwner")
                        .isEmpty())
                .andExpect(jsonPath("$.data[?(@.taskCode == 'GOODSHORT_DRAMA_CONTENT_SYNC')].title")
                        .value(hasItem("GoodShort 免费剧集同步")));
