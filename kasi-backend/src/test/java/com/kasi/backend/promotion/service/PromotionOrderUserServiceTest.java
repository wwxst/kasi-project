...
class PromotionOrderUserServiceTest extends BaseAuthTest {
    ...
    void monthlyQueryIsUserScopedAndRefundAware() {
        ...
        String csv = new String(userService.exportCsv(ids.primaryUserId(), query),
                java.nio.charset.StandardCharsets.UTF_8);

        assertThat(page.getList()).extracting("externalOrderId")
                .containsExactlyInAnyOrder("own-paid", "own-refunded");
        assertThat(monthly.getPaidOrderCount()).isEqualTo(1);
        assertThat(monthly.getGrossOrderAmount()).isEqualByComparingTo("9.99");
        ...
        assertThat(csv).startsWith("\uFEFF").contains("own-paid", "own-refunded")
                .doesNotContain("other-paid", "own-august", "平台ID", "用户ID", "归因状态");
    }
