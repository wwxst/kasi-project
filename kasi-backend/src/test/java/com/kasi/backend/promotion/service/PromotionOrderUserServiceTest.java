...
class PromotionOrderUserServiceTest extends BaseAuthTest {
    ...
    void monthlyQueryIsUserScopedAndRefundAware() {
        ...
        assertThat(page.getList()).filteredOn("externalOrderId", "own-refunded")
                .extracting("commissionAmount").containsExactly(BigDecimal.ZERO.setScale(2));
        assertThat(page.getList()).filteredOn("externalOrderId", "own-paid")
                .extracting("searchCode").containsExactly("code-own-paid");
    ...
    private void insertOrder(SeedIds ids, String orderId, Long userId, String status,
        ...
        jdbcTemplate.update("INSERT INTO promotion_order "
                        ...
                        + "search_code,tracking_no,raw_payload_json,sync_start_date,sync_end_date,last_synced_at) "
                ...
                commission == null ? null : new BigDecimal(commission), commissionStatus,
                "code-" + orderId, null, "{\"orderId\":\"" + orderId + "\"}");
    }
